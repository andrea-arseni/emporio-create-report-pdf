...
const getDate = (giornoEmissione) =>
    `${new Date(giornoEmissione).getDate()} ${
        MESI[new Date(giornoEmissione).getMonth()]
    } ${new Date(giornoEmissione).getFullYear()}`;
...
const disegnaIntestazione = (doc, logo) => {
    ...
    grad.stop(0, "#022D57", 1).stop(1, "#022D57", 0.7);
...
const displayNumeroPagina = (doc, numberPage, totalNumber) => {
    doc.fillColor(COLORS.NORMAL)
        ...
        .font("Helvetica")
        .text(
            "Pagina " + numberPage + " di " + totalNumber,
            doc.page.width - 116,
            doc.page.height - 50
        );
};
...
const displayFooter = (doc, firstPage = false) => {
    ...
    doc.fontSize(12).font("Helvetica").fillColor(COLORS.NORMAL);

    if (firstPage) {
        doc.text("Emporio Case sas", 250, doc.page.height - 87)
            .text(sede)
        ...
        doc.text("Emporio Case sas", 0, doc.page.height - 62, conf).text(
            sede,
            conf
        );
    }
...
const displayParagrafoKeyValueACapo = (
    doc,
    key,
    value,
    rows,
    startingHorizontalPoint
) => {
    doc.font("Helvetica-BoldOblique").text(key, startingHorizontalPoint);

    value.forEach((val) => {
        doc.font("Helvetica-Oblique").text(val, startingHorizontalPoint);
    });
...
const creaPrimaPagina = (doc, payload) => {
    ...
    grad.stop(0, "#022D57", 1).stop(1, "#022D57", 0.5);
    ...
    const textHeader =
        tipologia === "immobile"
            ? "Attività Immobile rif. " + immobile.ref
            : "Attività Agenzia";

    const textObject =
        tipologia === "immobile"
            ? [
                  "Analisi Annuncio e Valutazione Prezzo",
                  "Analisi Visite Effettuate",
                  "Analisi Foto e Documenti",
              ]
            : [
                  "Analisi Acquisizioni e Chiusure",
                  "Analisi Immobili Attivi",
                  "Elenco Visite Effettuate",
                  "Analisi Lavori Attivi",
              ];
    ...
    // Creato il
    displayParagrafoKeyValueACapo(
        doc,
        "Periodo:",
        [`Dal ${getDate(from)} al ${getDate(to)}`],
        3,
        250
    );
...
const rilevaPrezzoDiZona = (immobile, doc) => {
    console.log(immobile.datiZona);

    if (immobile.datiZona) {
        doc.font("Helvetica-Oblique")
            .text(
                `Disponibile prezzo medio di ${immobile.contratto} in zona a ${immobile.datiZona.periodoRiferimento}: `
            )
            .moveDown(1);
        let prezzoZona =
            immobile.datiZona.prezzoMetroQuadro +
            " (" +
            immobile.datiZona.range +
            ")";
        prezzoZona = prezzoZona.replace(/€/g, "€ ");
        prezzoZona = prezzoZona.replace(/m/g, " m");
        doc.fillColor(COLORS.BLUE).text(prezzoZona);
    } else {
        doc.font("Helvetica-Oblique")
            .fillColor(COLORS.RED)
            .text(
                "Analisi prezzo medio di " +
                    immobile.contratto +
                    " in zona non disponibile"
            )
            .fillColor(COLORS.NORMAL);
    }
...
const creaPaginaAnnuncioImmobile = (doc, payload) => {
    ...
    const logStato = immobile.logs
        .reverse()
        .find((el) => el.azione === immobile.status);
    const dataPubblicazione = logStato
        ? ` dal ${getDate(logStato.data)}`
        : " - mai stato attivato";

    // parte dichiarazione prezzo
    const prezzoMetroQuadro =
        formatNumber((immobile.prezzo / immobile.superficie).toFixed(0)) +
        " € / m² ";
...
const reportDocumentoPresent = (files, nome, doc, continued) => {
    ...
    dato
        ? doc
              ...
              .text("Presente", { continued })
              .fillColor(COLORS.NORMAL)
        : doc
              .fillColor(COLORS.RED)
              .text("Non Presente", { continued })
              .fillColor(COLORS.NORMAL);
    if (!continued) doc.moveDown(1);
};

const getNumeroFoto = (doc, files) => {
    doc.font("Helvetica-BoldOblique").text("Numero foto acquisite: ", {
        continued: true,
    });

    const numeroFoto = files.filter(
        (el) => el.tipologia === "FOTO" && el.nome !== "21"
    ).length;
    ...
    doc.font("Helvetica-Oblique")
        .text(numeroFoto + " su 20 possibili")
        .moveDown(1);
};
...
const creaPaginaDocumentiEFoto = (doc, payload) => {
    ...
    doc.font("Helvetica-BoldOblique").text("    Visura catastale: ", {
        continued: true,
    });
    reportDocumentoPresent(immobile.files, "visura catastale", doc, false);
    doc.font("Helvetica-BoldOblique").text("Atto provenienza: ", {
        continued: true,
    });
    reportDocumentoPresent(immobile.files, "atto provenienza", doc, true);
    doc.font("Helvetica-BoldOblique").text("    Consuntivo spese: ", {
        continued: true,
    });
    reportDocumentoPresent(immobile.files, "consuntivo spese", doc, false);
    doc.font("Helvetica-BoldOblique").text("Certificazione energetica: ", {
        continued: true,
    });
    reportDocumentoPresent(
        immobile.files,
        "certificazione energetica",
        doc,
        true
    );
    doc.font("Helvetica-BoldOblique").text(
        "    Identificativo proprietario: ",
        { continued: true }
    );
    reportDocumentoPresent(immobile.files, "identificativo", doc, false);
    doc.font("Helvetica-BoldOblique").text("Contratto collaborazione: ", {
        continued: true,
    });
    reportDocumentoPresent(
        immobile.files,
...
const creaPaginaModificheImmobile = (doc, payload) => {
    ...
    immobile.logs.sort(
        (firstEl, secondEl) =>
            new Date(firstEl.data).getTime() - new Date(secondEl.data).getTime()
    );
...
getPagine = (payload, pagina) => {
    ...
    if (payload.tipologia === "immobile") {
        ...
        if (numeroVisiteEccedenti > 0)
            numeroPagineTotali = Math.ceil(
                numeroPagineTotali + numeroVisiteEccedenti / 12
            );
        ...
        if (details) {
            ...
            if (numeroLogsEccedenti > 0)
                numeroPagineTotali = Math.ceil(
                    numeroPagineTotali + numeroLogsEccedenti / 16
                );
        }

        // retrieve current page
        switch (pagina) {
            case PAGINA.INTESTAZIONE:
                return { totali: numeroPagineTotali, corrente: 1 };
            case PAGINA.ANNUNCIO_IMMOBILE:
                return { totali: numeroPagineTotali, corrente: 2 };
            case PAGINA.VISITE_IMMOBILE:
                return { totali: numeroPagineTotali, corrente: 3 };
            case PAGINA.DOCS_IMMOBILE:
                return {
                    totali: numeroPagineTotali,
                    corrente: Math.ceil(4 + numeroVisiteEccedenti / 12),
                };
            case PAGINA.LOGS_IMMOBILE:
                return {
                    totali: numeroPagineTotali,
                    corrente: Math.ceil(5 + numeroVisiteEccedenti / 12),
                };
        }
    } else {
        const { acquisizioni, conclusi, attivi, visite, lavori, operazioni } =
            payload.dataOffice;
        ...
        const numeroImmobiliCambiatiEccedenti =
            acquisizioni.length + conclusi.length - 16;
        if (numeroImmobiliCambiatiEccedenti > 0)
            numeroPagineTotali = Math.ceil(
                numeroPagineTotali + numeroImmobiliCambiatiEccedenti / 16
            );

        // aggiunta pagine per immobili attivi eccedenti
        const numeroImmobiliAttiviEccedenti = attivi.length - 2;
        if (numeroImmobiliAttiviEccedenti > 0)
            numeroPagineTotali = Math.ceil(
                numeroPagineTotali + numeroImmobiliAttiviEccedenti / 3
            );
        ...
        lavori.forEach(
            (lavoro) =>
                (numeroStepsLavori =
                    numeroStepsLavori + 1 + lavoro.steps.length)
        );
        const numeroLavoriEccedenti = numeroStepsLavori - 10;
        if (numeroLavoriEccedenti > 0)
            numeroPagineTotali = Math.ceil(
                numeroPagineTotali + numeroLavoriEccedenti / 12
            );
        ...
        if (details) {
            ...
            if (numeroOperazioniEccedenti > 0)
                numeroPagineTotali = Math.ceil(
                    numeroPagineTotali + numeroOperazioniEccedenti / 16
                );
        }
        ...
        switch (pagina) {
            case PAGINA.INTESTAZIONE:
                return { totali: numeroPagineTotali, corrente: 1 };
            case PAGINA.ACQUISIZIONI_CHIUSURE:
                return { totali: numeroPagineTotali, corrente: 2 };
            case PAGINA.IMMOBILI_ATTIVI:
                return {
                    totali: numeroPagineTotali,
                    corrente:
                        3 + Math.ceil(numeroImmobiliCambiatiEccedenti / 16),
                };
            case PAGINA.VISITE_TOTALI:
                return {
                    totali: numeroPagineTotali,
                    corrente:
                        4 +
                        Math.ceil(numeroImmobiliCambiatiEccedenti / 16) +
                        Math.ceil(numeroImmobiliAttiviEccedenti / 3),
                };
            case PAGINA.LAVORI:
                return {
                    totali: numeroPagineTotali,
                    corrente:
                        ...
                        Math.ceil(numeroImmobiliCambiatiEccedenti / 16) +
                        Math.ceil(numeroImmobiliAttiviEccedenti / 3) +
                        Math.ceil(numeroVisiteEccedenti / 12),
                };
            case PAGINA.OPERAZIONI:
                return {
                    totali: numeroPagineTotali,
                    corrente:
                        6 +
                        ...
                        Math.ceil(numeroImmobiliAttiviEccedenti / 3) +
                        Math.ceil(numeroVisiteEccedenti / 12) +
                        Math.ceil(numeroLavoriEccedenti / 12),
                };
        }
    }
};
...
displayListaImmobili = (lista, doc, pagine, offset = 2) => {
    ...
    lista.forEach((el, index) => {
        ...
        doc.fontSize(13)
            ...
            .font("Helvetica-Oblique")
            .text(
                ` - (${getDate(el.data)}) Rif. ${el.ref}: ${
                    el.titolo.length > 46
                        ? el.titolo.substr(0, 40) + "..."
                        : el.titolo
                }`
            )
            .moveDown(1);
...
const creaPagineImmobiliAttivi = (doc, payload) => {
    ...
    creaHeader(doc, payload, [
        "Lista Immobili Attivi",
        "con Analisi Prezzo e Visite",
    ]);

    doc.font("Helvetica-BoldOblique")
        .fontSize(13)
        .text(
            `Numero immobili attivi al ${getDate(new Date())}:    ${
                attivi.length
            }`
        )
        .moveDown(3);
...
const displayVisite = (visite, doc, pagine, from, to) => {
    ...
    visite.sort(
        (firstEl, secondEl) =>
            new Date(firstEl.quando).getTime() -
            new Date(secondEl.quando).getTime()
    );
    ...
    visite.forEach((visita, index) => {
        ...
        const visitaText = visita.note
            ? `${visita.nome} (${visita.note})`
            : visita.nome;

        doc.fontSize(13)
            .font("Helvetica-BoldOblique")
            .text(
                `${++index}) ${getDate(visita.quando)} ${
                    visita.ref ? " per ref. " + visita.ref : ""
                }`,
                { continued: true }
            )
            .font("Helvetica-Oblique")
...
const creaPaginaLavori = (doc, payload) => {
    ...
    lavori.forEach((el, index) => {
        if (counter - 10 === 0 || (counter - 10) % 12 === 0)
            pagine = voltaPagina(doc, pagine);
        doc.font("Helvetica-BoldOblique")
...
const creaPaginaOperazioni = (doc, payload) => {
    ...
    operazioni.forEach((el, index) => {
        ...
        doc.font("Helvetica-BoldOblique")
            ...
            .text(
                `    ${el.importo > 0 ? "+" : ""}${formatNumber(el.importo)} €`,
                { continued: true }
            )
            .fillColor(COLORS.NORMAL)
            .font("Helvetica-Oblique")
    ...
    doc.font("Helvetica-BoldOblique")
        ...
        .fillColor(saldo > 0 ? COLORS.BLUE : COLORS.RED)
        .text(`    ${saldo > 0 ? "+" : ""}${formatNumber(saldo)} €`);
...
exports.creaReport = (payload) => {
    ...
    payload.tipologia === "immobile"
        ? creaReportImmobile(doc, payload)
        : creaReportGenerico(doc, payload);
    // chiudi documento
    doc.end();
    return doc;
};
